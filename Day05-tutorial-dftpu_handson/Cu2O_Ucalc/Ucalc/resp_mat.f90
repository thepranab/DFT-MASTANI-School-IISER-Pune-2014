...
!
      ...
      real(kind=8)   primv(3,3), atp_cry(3,200), atp_car(3,200),        &
                     ...
                     atp_cry_s(3,5000), daux, daux0, rm
     ...
     &         na_s(10), lmin
      integer  i, j, k, l, m, n, l1, l2, l3, i0
      integer, allocatable :: typ(:), typ_s(:), mol(:,:,:),             &
                              mol_s(:,:,:), i00(:,:), i000(:,:),        &
                              spin(:), spin_s(:), spinp(:,:,:),         &
                              spinp_s(:,:,:), spec(:), specord(:),      &
                              iord(:), rmi(:)
      ...
      namelist /input_mat/ ntyp, na, nalfa, filepos, back, n1, n2, n3,  &
                           filednda, filednbdab, filedndab, filednbda,  &
                           magn

      read(5,input_mat)

! please put reservoir as last type
...
! averages
      do j = 1,nat
         do l = 1,nat
!            if (j.eq.1) then
!               write(12,*) 'ciao ', l, dist(j,l)
!            end if
            if (chi(j,l).ne.0.d0) then
               nat0 = 1
               rm = chi(j,l)
               rmi(nat0) = j
               do k = 1,nat
                  if (typ(k).eq.typ(j).and.abs(dist(k,l)-dist(j,l)).le.6.d-3 &
     &                .and.dist(k,l).gt.0.d0.and.dist(j,l).gt.0.d0.and.&
     &                k.ne.j.and.chi(k,l).ne.0.d0) then !.and.  &
!     &                abs(chi(k,l)-chi(j,l)).gt.1.d-6) then
                     nat0 = nat0 + 1
                     rmi(nat0) = k
                     rm = rm + chi(k,l)
!                     write(12,*) 'quoooo ', j, k, l, chi(j,l), chi(k,l)
!                     chi(j,l) = 0.5d0*(chi(j,l)+chi(k,l))
!                     chi(k,l) = chi(j,l)
!                     chi0(j,l) = 0.5d0*(chi0(j,l)+chi0(k,l))
!                     chi0(k,l) = chi0(j,l)
                  end if
               end do
               do k = 1,nat0
                  chi(rmi(k),l) = rm/dfloat(nat0)
               end do
            end if
            if (chi0(j,l).ne.0.d0) then
               nat0 = 1
               rm = chi0(j,l)
               rmi(nat0) = j
               do k = 1,nat
                  if (typ(k).eq.typ(j).and.abs(dist(k,l)-dist(j,l)).le.6.d-3 &
     &                .and.dist(k,l).gt.0.d0.and.dist(j,l).gt.0.d0.and.&
     &                k.ne.j.and.chi0(k,l).ne.0.d0) then !.and.  &
!     &                abs(chi0(k,l)-chi0(j,l)).gt.1.d-6) then
                     nat0 = nat0 + 1
                     rmi(nat0) = k
                     rm = rm + chi0(k,l)
!                     write(12,*) 'quoooo ', j, k, l, chi(j,l), chi(k,l)
...
!                     chi0(k,l) = chi0(j,l)
      ...
      do j = 1,nat
         do l = 1,nat
!                     if (j.eq.3.and.l.eq.25) then
!                        write(12,*) 'quiiii ', dist(j,l), chi(j,l)
!                     end if
...
!               nat0 = nat0 + 1
 ...
 35            continue
               do k = 1,nat
                  if (chi(j,l).eq.0.d0.and.                             &
     &                (abs(dist(k,nat0)-dist(j,l)).le.6.d-3) .and.      &
     ...
     &                spin(j)*spin(l).eq.spin(nat0)*spin(k)) then
      ...
      write(12,*)
      do i = 1,nat+1
         write(12,'(8(x,f8.4))') (dist(i,j),j=1,nat+1)
      end do
      write(12,*)
            ...
            do nat0 = 1,nat
               if (typ(nat0).eq.j) then
                  if (i00(i,j).eq.0) then
                     ...
                     do k = i00(i,j),1,-1
                        if (abs(dist(i0,nat0)-d0(i,j,k)).le.6.d-3 & !qui
     &                      .and. spin(i0)*spin(nat0).eq.spinp(i,j,k)) then
...
!               chi0(nat+1,j) = chi0(j,nat+1)
         ...
         open(11,file=filednbda,status='unknown')
         do j = 1,ntyp
            read(11,*) filename(1,j), filename0(1,j)
         end do
         close(11)
         l = 0
         do i = 1,ntyp
            open(11,file=filename(1,i),status='unknown')
            do k = 1,nalfa
               read(11,*) alpha(k), nn(k)
            end do
            daux = (nn(nalfa) - nn(1))/(alpha(nalfa)-alpha(1))
            close(11)
            open(11,file=filename0(1,i),status='unknown')
            do k = 1,nalfa
               read(11,*) alpha(k), nn(k)
            end do
            daux0 = (nn(nalfa) - nn(1))/(alpha(nalfa)-alpha(1))
            close(11)
            do j = l+1,l+na(i)
               chi(nat+1,j) = daux
               chi0(nat+1,j) = daux0
...
! symmetrization
         do j = 1,nat
            chi(nat+1,j) = (chi(nat+1,j)+chi(j,nat+1))*0.5d0
            chi(j,nat+1) = chi(nat+1,j)
            chi0(nat+1,j) = (chi0(nat+1,j)+chi0(j,nat+1))*0.5d0
            chi0(j,nat+1) = chi0(nat+1,j)
         end do
         open(11,file=filednbdab,status='unknown')
!         do j = 1,nat
            read(11,*) filename(1,1), filename0(1,1)
!         end do
         close(11)
!         do i = 1,1
!            do j = 1,nat
               open(11,file=filename(1,1),status='unknown')
               do k = 1,nalfa
                  read(11,*) alpha(k), nn(k)
               end do
     ...
     &                       (nn(nalfa) - nn(1))/(alpha(nalfa)-alpha(1))
               close(11)
               open(11,file=filename0(1,1),status='unknown')
               do k = 1,nalfa
                  read(11,*) alpha(k), nn(k)
               end do
...
!      open(12,file='Umat.out',status='unknown')
      ...
      write(12,*)
      write(12,*) ' number of atoms in the primitive cell:', nat , nat*n1*n2*n3
      write(12,*) 
      if (nat.le.32) then
         write(12,*)
            ...
            do nat0 = 1,nat_s
               if (typ_s(nat0).eq.j) then
                  if (i000(i,j).eq.0) then
                     ...
                     do k = i000(i,j),1,-1
                        if (abs(dist_s(i0,nat0)-d0_s(i,j,k)).le.6.d-3 &
...
!!               write(12,*)
!!               write(12,*)                                              &
!!                 ' shell  #n: pc, sc, spinp   dist   chi_pc     chi_sc '
!               write(12,*)
!               do k = 1,i000(i,j)
!                  do m = 1,i00(i,j)
!                     if (abs(d0_s(i,j,k)-d0(i,j,m)).le.6.d-3.and.       &
...
!            end if
     ...
     &                 d0(typ_s(i),typ_s(k),l)).le.6.d-3 .and. &
 ...
 333           continue
               if (abs(dist_s(i,k)-d0(typ_s(i),typ_s(k),l)).le.6.d-3 .and. &
     &              spin_s(i)*spin_s(k).eq.spinp(typ_s(i),typ_s(k),l)) then
                  ...
                  chi0_s(i,k) = kp0(typ_s(i),typ_s(k),l)*                &
                                   dfloat(mol(typ_s(i),typ_s(k),l))/        &
                                   dfloat(mol_s(typ_s(i),typ_s(k),m))
!                  if (abs(chi_s(i,k)-0.000375).lt.1.d-6) then
!                     write(12,*) ' elemento inventato: ',i,k,chi(i,k), kp(typ_s(i),typ_s(k),l)
!                  end if
                  go to 14
               end if
            end do
...
!            chi0_s(i,nat_s+1) = chi0_s(nat_s+1,i)
            chi_s(nat_s+1,i) = chi(nat+1,k)
            chi_s(i,nat_s+1) = chi_s(nat_s+1,i)
            chi0_s(nat_s+1,i) = chi0(nat+1,k)
            chi0_s(i,nat_s+1) = chi0_s(nat_s+1,i)
         end do
      ...
      write(12,*) 
      write(12,*) 
      write(12,*) ' Sanity test and writing of response and interaction &
     &matrices '
      write(12,*) 
      write(12,*) ' atom1 ', ' atom2 ', ' typ1 ', ' typ2 ', ' dist ', ' &
     &chi_0 ', ' chi ', ' int '
      write(12,*) 
      
      allocate(disti(nat_s))
      allocate(distord(nat_s))
      allocate(spec(nat_s))
      allocate(specord(nat_s))
      allocate(iord(nat_s))
      spec(:) = typ_s(:)
      do i = 1,nat 
         disti(:) = dist_s(i,:)
...
! distord(1) e' la dist maggiore
         l = l+1
         distord(l) = distord(1)
         specord(l) = specord(1)
         iord(l) = iord(1)
         do j = 1,nat_s
            if (disti(j).gt.0.d0.and.disti(j).lt.distord(l)) then
               distord(1) = disti(j)
               specord(1) = spec(j)
               iord(1) = j
               go to 55
 ...
 55      continue
         do j = 1,nat_s
            if (disti(j).gt.distord(l)) then
               l = l+1
               distord(l) = disti(j)
               specord(l) = spec(j)
               iord(l) = j
            else if (disti(j).le.distord(1).and.j.ne.iord(1)) then
               lmin = 2
               l = l+1
               do k = l,lmin,-1
                  distord(k) = distord(k-1)
                  specord(k) = specord(k-1)
                  iord(k) = iord(k-1)
               end do
               distord(1) = disti(j)
               specord(1) = spec(j)
               ...
               do k = l-1,1,-1
               if (disti(j).gt.distord(k).and.disti(j).le.distord(k+1)) then
                  if (j.ne.iord(k+1)) then
                     lmin = k + 2
                     l = l+1
                     do m = l,lmin,-1
                        distord(m) = distord(m-1)
                        specord(m) = specord(m-1)
                        iord(m) = iord(m-1)
                     end do
                     distord(k+1) = disti(j)
                     specord(k+1) = spec(j)
  ...
  75           continue
         ...
         do j = 1,nat
            daux = 0.d0
            do k = 1,i00(typ(i),specord(j))
               if (abs(dist_s(i,iord(j))-d0(typ(i),specord(j),k)).lt.6.d-3) then
                  daux = kp(typ(i),specord(j),k)
                  do m = 1,i000(typ(i),specord(j))
                     if (abs(dist_s(i,iord(j))-d0_s(typ(i),specord(j),m)).lt.6.d-3) then
                        go to 77
                     end if
                  end do
               end if
  ...
  77        continue  
            if (abs(daux*dfloat(mol(typ(i),specord(j),k))/dfloat(mol_s(typ(i),specord(j),m))-chi_s(i,iord(j))).gt.1.d-5) then
               write(12,*) 'problem (see the following line:  i  j  typj&
     &mol mol_s dists chi chi_s)! '
               write(12,'(x,5(i3,x),f12.6,x,f12.6,x,f12.6)') i, iord(j),&
     & specord(j), mol(typ(i),specord(j),k), mol_s(typ(i),specord(j),m), dist_s(i,iord(j)), daux, chi_s(i,iord(j))
            else
               write(12,'(x,4(i3,x),f12.6,x,3(f10.4,x))') i, iord(j),&
     &              typ(i), specord(j), dist_s(i,iord(j)),              &
     &              chi0_s(i,iord(j)), chi_s(i,iord(j)),                &
     &              chi0m1_s(i,iord(j))-chim1_s(i,iord(j))
            end if
         end do
         write(12,'(x,3(i3,x),a4,x,f12.6,x,3(f10.4,x))') i, nat_s+1,    &
     &        typ(i), ' back', 0.0, chi0_s(i,nat_s+1), chi_s(i,nat_s+1), &
     &        chi0m1_s(i,nat_s+1)-chim1_s(i,nat_s+1) 
         write(12,*) 'end of test on atom ', i
      end do
...
! Let's write the result for U in the supercell.
      ...
      if (nat_s.le.3200) then
         write(12,*)
         write(12,*) ' CHI_0 Matrix '
         write(12,*)
         do i = 1,nat+1 !nat_s+1
            write(12,'(8(x,f8.4))') (chi0_s(i,j),j=1,nat+1) !nat_s+1)
         end do
         write(12,*)
         ...
         write(12,*)
         do i = 1,nat+1 !nat_s+1
            write(12,'(8(x,f8.4))') (chi_s(i,j),j=1,nat+1) !nat_s+1)
         end do
         write(12,*)
...
!            write(12,*) (chi0m1(i,j),j=1,nat+1)
            write(12,'(8(x,f9.4))') (chi0m1_s(i,j)-chim1_s(i,j),j=1,nat+1) !nat_s+1)
         end do
      else
         write(12,*)
         write(12,*) ' CHI0 Matrix '
...
! ipiv  : work space for pivoting
      ...
      do i = 1,n
         do j = 1,n
!            if (i.le.4.and.j.le.4) write(6,*) 'ciao: ', i, j, a(i,j)
         end do
      end do
